...
openssl req -new -key %CERTS_DIR%\%dname%.key -out %TMP_DIR%\%dname%.csr -subj /emailAddress=%KEY_EMAIL%/C="%KEY_COUNTRY%"/stateOrProvinceName="%KEY_STATE%"/L="%KEY_CITY%"/O="%KEY_ORG%"/OU="%KEY_ORG_UNIT%"/CN=%dname% -passout pass:%PFX_PASS%
rem Для создания самоподписанного сертификата
rem openssl x509 -sha256 -req -days %VALID_DAYS% -in %TMP_DIR%\%dname%.csr -signkey %CERTS_DIR%\%dname%.key -out %CERTS_DIR%\%dname%.crt -passin pass:%PFX_PASS%
rem Для создания сертификата, подписанного доверенным сертификатом
openssl x509 -req -days %VALID_DAYS% -in %TMP_DIR%\%dname%.csr -extfile %TMP_DIR%\%dname%.cnf -extensions trust_cert -CA %ECC_CA_DIR%/trusted.crt -CAkey %ECC_CA_DIR%/trusted.key -out %CERTS_DIR%\%dname%.crt 
